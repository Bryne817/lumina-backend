...
test('search ranks by trigram similarity, not recency', async () => {
  ...
  await searchTransactions(pool, { query: 'hello', limit: 20 });
...
test('an exact case-insensitive memo always outranks a fuzzy match', async () => {
  ...
  await searchTransactions(pool, { query: 'ORDER-4471', limit: 20 });
...
test('search uses the trigram operator so the GIN index applies', async () => {
  ...
  await searchTransactions(pool, { query: 'invoice', limit: 20 });
...
test('search skips transactions with no memo', async () => {
  ...
  await searchTransactions(pool, { query: 'x', limit: 20 });
...
test('the query is bound as a parameter, never interpolated', async () => {
  ...
  await searchTransactions(pool, { query: hostile, limit: 20 });
...
test('an empty or whitespace query is rejected', async () => {
  ...
  await assert.rejects(() => searchTransactions(pool, { query: '   ', limit: 20 }), SearchError);
  await assert.rejects(() => searchTransactions(pool, { query: '', limit: 20 }), SearchError);
});
...
test('an absurdly long query is rejected rather than run', async () => {
  ...
  await assert.rejects(
    () => searchTransactions(pool, { query: 'x'.repeat(MAX_QUERY_LENGTH + 1), limit: 20 }),
    SearchError
  );
});
...
test('search returns the cursor for the next page from the last row', async () => {
  ...
  const { nextCursor } = await searchTransactions(pool, { query: 'o', limit: 2 });
...
test('an exhausted search returns no cursor', async () => {
  ...
  const { items, nextCursor } = await searchTransactions(pool, { query: 'nothing', limit: 20 });
...
test('paging keysets on the whole ranking tuple, so a page boundary is stable', async () => {
  ...
  await searchTransactions(pool, { query: 'hello', limit: 20, cursor });
  ...
  assert.deepEqual(calls[0].params.slice(2, 5), [0.5, 900, 'zzz']);
});
...
test('an invalid cursor is rejected rather than silently ignored', async () => {
  ...
  await assert.rejects(
    () => searchTransactions(pool, { query: 'x', limit: 20, cursor: 'garbage!!' }),
    SearchError
  );
});
...
test('getOperationsByAsset scopes, orders and limits', async () => {
  ...
  await getOperationsByAsset(pool, { asset: `USDC:${ISSUER}`, limit: 10 });
...
test('an asset filter composes with account and type filters', async () => {
  ...
  assert.match(calls[0].sql, /source_account = \$3/);
  assert.match(calls[0].sql, /type = \$4/);
  // Types are stored lowercase but the GraphQL enum is uppercase.
  assert.equal(calls[0].params[3], 'payment');
});

test('an asset query paginates by keyset rather than offset', async () => {
  const { pool, calls } = fakePool([]);

  await getOperationsByAsset(pool, { asset: 'XLM', limit: 5, cursor: 'op99' });
...
test('an invalid asset is rejected before any query runs', async () => {
  ...
  await assert.rejects(() => getOperationsByAsset(pool, { asset: 'USDC', limit: 5 }), SearchError);
