...
test('notifyIndexed sends the payload as a bind parameter, never inlined', async () => {
  ...
  await notifyIndexed(db, { kind: 'ledger', ledger: 100, transactions: 2, operations: 5 });
...
test('serializeNotification keeps a normal payload intact', () => {
  const payload = serializeNotification({ kind: 'ledger', ledger: 42, transactions: 1, operations: 3 });

  assert.deepEqual(JSON.parse(payload), { kind: 'ledger', ledger: 42, transactions: 1, operations: 3 });
});
...
test('serializeNotification degrades rather than exceeding the NOTIFY limit', () => {
  ...
  const payload = serializeNotification(huge as never);

  assert.ok(Buffer.byteLength(payload, 'utf8') < 8000);
  assert.deepEqual(JSON.parse(payload), { kind: 'ledger', ledger: 99 });
});

test('parseNotification round-trips what notifyIndexed writes', () => {
  const original = { kind: 'events' as const, ledger: 7, events: 3 };
